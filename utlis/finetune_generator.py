...
def get_train_loader(metatrain_character_folders, batchsize, class_num):
    dataset = pu_train(metatrain_character_folders, class_num)
    loader = DataLoader(dataset, batch_size=batchsize, shuffle=True)
    return loader


def get_test_loader(task, num_per_class=1, split='support', num_classes=1):
    dataset = pu_test(task, split)
    if split == 'support':
